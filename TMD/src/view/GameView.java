...
public class GameView extends JPanel {
    ...
    public GameView(MainFrame frame) {
        ...
        // Presenter sengaja dibiarkan null di sini sampai resetGameSesi() dipanggil
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        // PERBAIKAN: Tambahkan pengecekan null agar tidak crash saat game pertama kali load
        if (presenter != null) {
            drawGame(g2);
        } else {
            // Opsional: Gambar loading screen atau biarkan kosong (hanya background)
            g2.setColor(Color.WHITE);
            g2.drawString("Loading Game...", getWidth()/2 - 40, getHeight()/2);
        }
    }

    private void drawGame(Graphics2D g) {
        // Karena sudah dicek null di paintComponent, memanggil getPlayer() di sini sekarang aman
        ...
        g.drawString(status, (getWidth() - sWidth) / 2, 80);
    ...
    public void resetGameSesi() {
        // Ambil nama dari frame dan buat presenter baru
        String name = frame.getPlayerName();
        this.presenter = new GamePresenter(this, name);
    }
